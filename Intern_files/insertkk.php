...
<?php
	...
	if (isset($_GET['konto']))
	{
		...
		$result = $con->execute("INSERT into kontobewegungen (transaktions_id,banktrans_id,datum,jahrmonat,konto,betrag,text,erstellt,status) values (" . $tid . "," . $id . ",'" . $datum . "'," . $jahrmonat . ",30000,'" . $betrag . "','" . $text . "','" . $erstellt . "','B')");
		if ($result)
		{
			$betrag = $betrag * -1;
			...
			{
				$res2 = $con->execute("UPDATE kreditkarte set status='V' where kreditkarte_id=" . $id);
				if (!$res2)
				{
					echo 'Fehler in der Abfrage. ' . htmlspecialchars($result->errorinfo()[2]);
				}
