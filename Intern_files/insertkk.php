...
<?php
	...
	if (isset($_GET['konto']))
	{
		...
		$result = mysql_query("INSERT into kontobewegungen (transaktions_id,banktrans_id,datum,jahrmonat,konto,betrag,text,erstellt,status) values (" . $tid . "," . $id . ",'" . $datum . "'," . $jahrmonat . ",30000,'" . $betrag . "','" . $text . "','" . $erstellt . "','B')");
		if ($result)
		{
			$betrag = $betrag * -1;
			...
			{
				$res2 = mysql_query("UPDATE kreditkarte set status='V' where kreditkarte_id=" . $id);
				if (!$res2)
				{
					echo "MySQL Error (" . mysql_errno() . "): " . mysql_error();
				}
	...
	else
	{
		...
		echo "<br>";
		echo "<form name = \"kkdetails\" action = \"insertkk.php\" >";
		echo "<table>";
		...
		echo "</table>";
		echo "<br>";
		echo "<input type=hidden name=\"kreditkarte_id\" value=" . $id . " />";
